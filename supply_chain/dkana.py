...
def build_mfsc_relational_state(
    ...
    *,
    ...
    return build_equality_row_matrix(all_values, variable_names, enumeration_map)
...
def build_dkana_windows(
    *,
    ...
    row_matrices = np.stack(
        [
            build_mfsc_relational_state(
                observations_array[index],
                state_constraint_array[index],
    ...
    return DKANADataset(
        ...
        config_fields=DKANA_CONFIG_FIELDS,
...
class DKANAPolicy(nn.Module):
    ...
    def forward(
        ...
        if time_mask is None:
            ...
            last_valid_index = time_mask.to(dtype=torch.int64).sum(dim=1) - 1
