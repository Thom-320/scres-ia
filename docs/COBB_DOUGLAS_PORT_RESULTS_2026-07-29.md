...
The separate dynamic-arm development screen is generated by
`scripts/score_dynamic_arms_cobb_douglas.py` and stored in
`results/cobb_douglas/dynamic_arms_v2.json`.
...
## 7. Dynamic-arm development screen

The index was subsequently applied to `ddmrp_projected_v2` on four new development
tapes per risk family. DDMRP is projected onto the same discrete `6^3 = 216`
three-node action domain as the static policies. MPC is deliberately absent: its
concurrent state-replay experiment owns different outputs and is not duplicated here.

The declared comparison set contains the 216-posture buffer-gate incumbent
`(672,0,1344)` (targets `61,440/0/126,000`), the one-tape v2-preflight winner
`(168,0,168)`, two homogeneous references, the zero posture, and projected DDMRP.
Because κ̇ is set-relative, these R_CD values apply only to this six-arm set.

### R1r dynamic panel

| policy | R_CD | ret_excel | full ledger | fill | delivered | κ | injected |
|---|---:|---:|---:|---:|---:|---:|---:|
| **(168,0,168)** | **0.5642** | **0.005568** | **0.005547** | 0.99634 | 680,453 | 388,544 | 329,853 |
| (168,168,168) | 0.5640 | 0.005486 | 0.005468 | 0.99658 | 727,916 | 387,496 | 398,880 |
| ddmrp_projected_v2 | 0.5560 | 0.005469 | 0.005451 | **0.99669** | **752,689** | 424,248 | 1,040,894 |
| (672,0,1344) | 0.5434 | 0.005494 | 0.005475 | 0.99661 | 734,102 | 497,972 | 572,289 |
| (1344,1344,1344) | 0.5419 | 0.005469 | 0.005451 | 0.99669 | 752,689 | 501,484 | 1,119,644 |
| (0,0,0) | 0.4661 | 0.004547 | 0.003463 | 0.75441 | 518,656 | 464,903 | 0 |

The DDMRP arm has the highest fill and delivery but does not lead any of the three
resilience metrics. R_CD makes the trade-off legible: relative to `(168,0,168)`,
DDMRP injects 3.16 times as much strategic material and has 9.2% greater κ for
10.6% more delivery. This is a declared cost-versus-service judgment, not a service
guarantee.

### R2r metric disagreement

`(168,0,168)` leads R_CD (`0.5513`) and ret_excel (`0.478414`). In contrast,
ret_excel_full_ledger places DDMRP and `(1344,1344,1344)` jointly first
(`0.275581`). This is metric-dependent development evidence, not a DDMRP victory.

### Auditable DDMRP mechanism

All 104 DDMRP decisions are persisted. In R1r the controller selects
`(1344,1344,504)` in 52/52 epochs. In R2r it selects `(1344,1344,504)` in
41/52 and `(1344,1344,336)` in 11/52. Thus only the rations-buffer component
changes in this screen; the two raw-material components remain at the maximum.

DDMRP and `(1344,1344,1344)` have identical per-tape ReT, full-ledger ReT,
ret_thesis, fill, delivery, and lost-order outcomes in both families. The defensible
statement is local: on these tapes, changing the rations target among the selected
upper rungs did not alter those physical endpoints. It does not establish that
`op9_rations` is globally transition-dead.

The earlier draft incorrectly called `(168,0,168)` the 216-posture gate incumbent
and reported unaudited 14/12 action counts. Both statements are superseded by the
persisted v2 result.

## 8. What this does not establish

Development tapes only, one risk corner per family, four tapes per cell, and no
confirmatory intervals. The six-arm C-D set is not a fresh enumeration of all 216
postures on these tapes. Nothing here adjudicates `H_obs`, MPC, a learner, or neural
premium.

## 9. One thing the 2024 paper gives us
