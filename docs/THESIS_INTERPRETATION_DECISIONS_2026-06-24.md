...
| D5 | Raw-material quantities (Op2 190k, Op3 15.5k, Table 6.16 buffer) | Per individual raw material `rm1..rm12`; aggregate representation must preserve 12 component-units per ration-kit | **MATCHES-PUBLISHED** (corregido 2026-08-09; era CHOSEN-AMBIGUOUS) | Thesis text: Op2 handles 190,000 of *each* rm; Op3 sends 15,500 of *each*; same `I_{t,S}` level applied at Op3/Op5/Op9. |
...
## Corrección de D5 — 2026-08-09

**D5 estaba etiquetada `CHOSEN-AMBIGUOUS` y no lo era.** La Tabla 6.20 de Garrido-Ríos (2017)
escribe, con esas palabras, «190,000 units of **each rm**» para Op2 y «15,500 units of **each rm**»
para Op3. No hay ambigüedad que resolver: la lectura por materia prima **está publicada**.

La etiqueta se corrige a `MATCHES-PUBLISHED`. La lectura alternativa —190.000 como total entre las
doce— se corrió igualmente y queda **refutada por la física**: el servicio cae de 0,6777 a 0,4015 y
la cadena termina con cero unidades en mano. Artefacto:
`results/procurement_overorder_source/result.json`.

**Y la tabla dice algo más que no estaba registrado en ninguna decisión.** Op2 es **idéntico** en
S = 1, 2 y 3, mientras Op3 y Op4 escalan con los turnos (15.500 / 31.000 / 47.000 por rm). Es decir:
**el aprovisionamiento está dimensionado para alimentar la capacidad completa de tres turnos y se
mantiene fijo mientras S es el tratamiento.**
...
El excedente de 3× a un turno **no es un descuido ni un artefacto nuestro: es la consecuencia
aritmética de mantener constante una variable de control a lo largo del diseño experimental**. Por
eso la materia prima nunca puede ser la restricción activa a S = 1, y por eso reducir el volumen
contratado para que un proveedor importe rompería el experimento al que ese parámetro pertenece.
