...
| C25 | The old Ruta B pre-risk splice gate is not valid prevention evidence; Gate v2 is implemented and the first forced-prep ceiling screen does not find preventive headroom in selected Case C. | **Supported as a negative/methodological boundary** | `docs/TRACK_B_RUTA_B_COUNTERFACTUAL_GATE_AUDIT_2026-07-07.md` retracts the 74.1% Ruta B positive-pair result because reactive PPO and permuted-label controls also pass the splice/calm-action gates. `docs/TRACK_B_PREVENTION_GATE_V2_IMPLEMENTATION_2026-07-07.md` implements full-rerun risk-event-tape Gate v2 (`forced_prep_sweep`, `event_on_off`, `oracle_warning`). First selected Case C forced-prep screen: `outputs/experiments/track_b_prevention_gate_v2_case_c_screen_2026-07-07`, 3 seeds x 8 eval episodes, 96 pairs per prep comparison, R24/R22 anchors, `high_dispatch_minus_medium` and `combined_prep_minus_medium` both mean local Excel-ReT delta `0.0`, positive rate `0.0`, `prevention_headroom_found=false`. | Do not cite Ruta B as preventive. Do not train a preventive policy unless a richer Gate-v2 ceiling assay first finds positive local event-level headroom and placebos/reactive-null controls remain null. |
