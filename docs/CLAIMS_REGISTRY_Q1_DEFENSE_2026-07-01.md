...
| C7 | RL value is frontier-dependent: action-space alignment with the downstream bottleneck explains Track A vs Track B. | **Supported** | 5-seed/60k final ablation (2026-07-02, `docs/E4_ABLATION_VERDICT_2026-07-02.md`): `downstream_only` ΔReT vs best static (+0.000566) exceeds `joint` (+0.000415) and `shift_only` (+0.000416) — win is not explained by action-space size, concentrates in downstream dispatch access. | Keep joint 8D contract as the primary canonical result; cite ablation as mechanism evidence only. Note the ablation used a different LR (1e-4) than canonical (3e-4) — within-arm comparison valid, absolute numbers not directly comparable to canonical headline. |
...
| C11 | Track B generalizes beyond one adaptive benchmark cell. | **Supported, with one disclosed exception** | Cross-regime/horizon matrix (2026-07-02, `docs/E3_GENERALIZATION_VERDICT_2026-07-02.md`): PPO wins 5/6 cells across current/increased/severe × h52/h104 (deltas up to +0.000742, exceeding the canonical cell); loses narrowly at severe/h52 (deepest-floor regime, fill ~0.1% for all policies). R2/R24/mixed families and h260 still not covered. | Report the full 6-cell table with the severe/h52 exception disclosed; do not claim "all risk levels." |
