...
For Paper 1, **C30 is the terminal same-contract gate and supersedes any
adaptive-advantage or bottleneck-value reading of C1–C7, C11, and C16–C21.**
Those entries remain as provenance for restricted comparator families, not as
current headline claims.

| ID | Claim | Status | Current evidence | Before submission |
|---|---|---|---|---|
| C30 | A strong calibration-only constant policy spanning the same full Track B contract matches or exceeds canonical PPO; the earlier PPO advantage is comparator-family dependent. | **Supported; terminal gate FAILS the adaptive-advantage claim (2026-07-10)** | Frozen protocol, calibration tapes `300001–300024`, final tapes `400001–400060`. Full-contract static Excel ReT `0.005906366`; canonical 10-checkpoint PPO `0.005888317`; PPO−static `−0.000018049`, two-way CI95 `[−0.000028615,−0.000008087]`, 2/10 checkpoint means and 2/60 tape means positive. CVaR05 agrees (`−0.000101921`, CI95 `[−0.000163278,−0.000042639]`). A small within-learner dispatch increment remains: fresh joint PPO−upstream/shift PPO anchored at Op10 `2.0x`, Op12 `1.5x` = `+0.000021106`, CI95 `[+0.000007015,+0.000034963]`, 4/5 seeds. See `docs/TRACK_B_SAME_CONTRACT_CHALLENGE_VERDICT_2026-07-10.md`. | Retire “PPO beats strong static,” general adaptive-advantage, and IJPR-leading bottleneck language. Reframe Paper 1 as an adversarial benchmark/comparator-contract audit; C&IE primary, SMPT backup. Preserve older positive numbers only with an explicit restricted-frontier qualifier. No further static optimization. |
| C29 | In the engineered R21 compounding-starvation regime, a frozen heterogeneous reserve posture plus adaptive 8D PPO reproduces the full 11D buffer-contract gain; weekly dynamic buffer control is unnecessary. | **Supported and confirmed at 5 seeds x 60k** | No-buffer 8D `0.311676`; dynamic 11D `0.340164`; fixed-posture 8D `0.340605`. Fixed-minus-11D `+0.000440`, seed-clustered CI95 `[−0.000799,+0.001680]` — "no detected difference at current precision"; NOT formal equivalence (no margin/TOST was prespecified). Fixed-minus-no-buffer `+0.028928`, CI95 `[+0.016283,+0.041574]`, 5/5 seeds and 117/120 episodes positive. This independently reproduces the 11D-minus-8D contract increment (`+0.028488`). Same-checkpoint and disjoint-calibration controls also find no scheduling value. | Keep outside Paper 1. Paper 2 should frame this as two-stage reserve design plus adaptive recovery, never anticipation or dynamic preventive RL. Because the posture was distilled from 11D, a held-out classical per-op frontier is still needed before claiming RL is the superior posture optimizer. Route-aware replenishment and actual-inventory holding sensitivity remain mandatory. |
...
| C7 | RL value is frontier-dependent: adaptive control of levers the static family holds fixed carries most of the Track B gain, and adding downstream dispatch authority contributes a causally identified further increment. | **Supported and IDENTIFIED (2026-07-10 corrected factorial); proportions revised vs the old E4 reading** | 5-seed/60k final ablation (2026-07-02, `docs/E4_ABLATION_VERDICT_2026-07-02.md`). Two comparator definitions, same ordering: vs best STATIC in-arm, downstream_only +0.000566 > joint +0.000415 ≈ shift_only +0.000416; vs best EVALUATED comparator incl. heuristics (the stronger bar, used in the manuscript), downstream_only +0.000429 > joint +0.000367 ≈ shift_only +0.000377. Win is not explained by action-space size; concentrates in downstream dispatch access. | Manuscript uses the heuristic-inclusive definition (stronger). Keep joint 8D as the canonical result; ablation is mechanism evidence only. Ablation LR (1e-4) differs from canonical (3e-4) — within-ablation comparison valid, absolute numbers not comparable to canonical headline. CORRECTION 2026-07-09 (verified): E4 arm names were misdescribed — "shift_only" freezes ONLY dims 6-7 (keeps upstream 0-5), "downstream_only" freezes ONLY dim 5 (keeps upstream 0-4 + dispatch); both arms retain upstream inventory control and each used a different comparator. E4 is pattern evidence, not causal identification. The identified test is the corrected common-comparator factorial (`run_track_b_contract_factorial.py`: joint vs upstream_shift vs dispatch_only, common tapes 200001+, lr 3e-4) — pending; FACTORIAL VERDICT 2026-07-10 (`docs/TRACK_B_CONTRACT_FACTORIAL_VERDICT_2026-07-10.md`, runs `track_b_factorial_*_2026-07-09`): PRIMARY joint−upstream_shift = +0.000092 two-way CI95 [+0.000077,+0.000108], 5/5 seeds → gate PASSES, mechanism identified. Decomposition vs common static on fresh tapes: joint +0.000517; upstream_shift +0.000425 (~82% of joint); dispatch_only −0.000012 (null — the 147-cell frontier already optimizes that subspace). E4's "downstream is the strongest lever" reading is retired; correct language: adaptive value concentrates where the comparator is static, dispatch access adds an identified increment. Joint arm independently replicates the headline on fresh tapes. ANCHOR CAVEAT 2026-07-10 (verified): upstream_shift froze dispatch at NEUTRAL 1.25x/1.25x (decoder 1.25+0.75x), not the best fixed 2.0x/1.5x, so +0.000092 = dispatch access vs neutral fixed dispatch, NOT vs best fixed dispatch; dispatch_only froze upstream at 1.25x while the statics use base 1.0x (near-subspace null). Same-contract static challenge running (docs/TRACK_B_SAME_CONTRACT_CHALLENGE_PROTOCOL_2026-07-10.md: full-contract calibration-only static search + upstream_shift_best_dispatch arm, virgin tapes 400001+, frozen stop rule); consolidated contrasts: scripts/aggregate_track_b_factorial.py -> outputs/experiments/track_b_factorial_aggregate_2026-07-09/. |
...
| C21 | Track B headline result holds at 10 training seeds, not just 5. | **Supported** | `docs/TRACK_B_SEED_EXPANSION_6_10_VERDICT_2026-07-02.md` + `docs/track_b_q1_stats_2026-07-02_final_10seed/`: seeds 6-10 CRN-paired against the exact canonical best static (S2_op10_2.00_op12_1.50, 147-cell convention), 120 total pairs. Authoritative Excel ReT delta +0.000438, CI95 [+0.000409,+0.000468], seed-clustered CI95 [+0.000421,+0.000458], 10/10 seeds positive; order-level ReT delta +0.000426. CROSSED HELD-OUT UPGRADE 2026-07-09 (Blocker 1, external-assessment response): all 10 frozen checkpoints + the prespecified static comparator evaluated on 60 FRESH tapes (eval seeds 200001-200060, never used before): Excel ReT delta +0.000486, two-way (checkpoint x tape) bootstrap CI95 [+0.000456,+0.000517]; 10/10 checkpoints positive, 60/60 tapes positive, min leave-one-out +0.000479; CVaR05 delta +0.001637 [+0.001530,+0.001743]. This answers the 21-unique-tape pseudoreplication critique with a strictly stronger result. Obs heterogeneity disclosed: seeds 1-5 trained on 48-dim v7, 6-10 on 52-dim (tail fields appended between runs; exact slicing adapter). `outputs/experiments/track_b_crossed_eval_2026-07-09/`. | Cited in manuscript sections/04_results.tex §4.3; Table 4 now uses the 10-seed headline comparison. |
...
| C28 | In the engineered R21 compounding-starvation regime, strategic-buffer headroom is real but weekly dynamic buffer control is unnecessary; a two-stage fixed reserve posture plus adaptive 8D PPO matches or exceeds the 11D policy. | **Supported at screen scale for the two-stage mechanism; 5-seed fixed-posture confirmation pending** | Physical headroom appears only in the high-frequency/high-impact region: always-prepared minus never-prepared `+0.0299`, CI95 `[+0.0142,+0.0478]`; natural-to-moderate cells are null. Confirmatory PPO-11D minus no-buffer PPO-8D is `+0.028488`, CI95 `[+0.015813,+0.041163]`, 5/5 seeds. Same-checkpoint and disjoint-calibration controls find no scheduling value: dynamic `0.340164`, globally frozen `0.340321`, self-minus-frozen CI95 `[−0.000746,+0.000433]`. The decisive retrained screen uses the calibrated `(Op3,Op5,Op9)=(0.1531,0.2480,0.2068)` posture with an 8D PPO: fixed `0.339790` versus matched 11D `0.333956`, delta `+0.005834`, seed-clustered CI95 `[+0.001146,+0.010522]`, 3/3 seeds; fixed versus no-buffer 8D `+0.059109`, CI95 `[+0.031274,+0.086943]`. R11 and timing claims are retracted. Holding-price significance is clean only near `lambda_h=0.05`, crossover about 0.138. | Keep outside Paper 1. Paper 2 should frame the mechanism as two-stage reserve design plus adaptive recovery, not anticipation or dynamic preventive RL. The posture was distilled from 11D learners, so a held-out fixed per-op frontier is required before claiming RL is superior to classical reserve optimization. Real-KAN-11D remains a sidecar. Sources: `docs/TRACK_BP_GATE2_SCREEN_VERDICT_2026-07-09.md`, `outputs/experiments/track_bp_frozen_posture_audit_5seed_2026-07-09/`, and `outputs/experiments/track_bp_fixed_posture_8d_3seed_30k_2026-07-09/`. |
