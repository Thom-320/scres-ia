...
| ⭐1b-preventive-excel-delta | **continuous_its × risk_obs+hazard × ReT_excel_delta × resource-aware Pareto × war φ4/ψ1.5, h104** | TOP CURRENT WIN, KAGGLE MIXED-10/60K CONFIRMED | 5-seed/40k, 5-seed/60k, mixed-10/40k, mixed-10/60k local, and Kaggle mixed-10/60k all confirmed fair resource-aware win. Kaggle final: dynamic Excel 0.002142, CVaR 5.22e9, resource 0.241, `frac_std=0.257`; Pareto win on both Excel and CVaR; `primary_win=true`. Action audit shows risk/hazard features among top drivers across seeds (`active/recent_R12/R22/R23/R24`, `weeks_since_last_R1/R3`) plus inventory/time terms. This is now the lead paper lane. Artifacts: `outputs/experiments/preventive_pareto_hc0.0_2026-06-27`, `outputs/experiments/preventive_pareto_excel_delta_5seed_40k_2026-06-27`, `outputs/experiments/preventive_pareto_excel_delta_5seed_60k_2026-06-28`, `outputs/experiments/preventive_pareto_excel_delta_mixed10seed_40k_2026-06-28`, `outputs/experiments/preventive_pareto_excel_delta_mixed10seed_60k_2026-06-28`, `outputs/kaggle/scresia-preventive-pareto-final/scresia_preventive_pareto_final_outputs/decision.json`, `outputs/experiments/preventive_pareto_results_summary_2026-06-28.json`. |
...
1. ⭐ continuous_its × v6 reward finalists at horizons `104` and then `260` (longer horizon may let preparation pay).
2. Track B only after fixing/understanding compressed fill scale and 7D heuristic mismatch.
3. ⭐ continuous_its × risk_obs+hazard × resource-aware Pareto × ReT_excel_delta × war, package Kaggle final and draft the claim boundary/results table.
...
- **Δmemory (ReT_retained − ReT_reset):** checked ONLY on Discrete(18) (control_v1/CD, masked) →
  null. **NEVER on the promising continuous_its+v6+h104 lane.** Runner
  `evaluate_retained_reset_learning.py:431` is hardwired to `make_discrete18_track_a_env` → needs
  extension to test memory where the policy actually learns. OPEN GAP.
