...
| ⭐1b-preventive-excel-delta | **continuous_its × risk_obs+hazard × ReT_excel_delta × resource-aware Pareto × war φ4/ψ1.5, h104** | TOP CURRENT WIN, KAGGLE MIXED-10/60K CONFIRMED | 5-seed/40k, 5-seed/60k, mixed-10/40k, mixed-10/60k local, and Kaggle mixed-10/60k all confirmed fair resource-aware win. Kaggle final: dynamic Excel 0.002142, CVaR 5.22e9, resource 0.241, `frac_std=0.257`; Pareto win on both Excel and CVaR; `primary_win=true`. Mechanism audit (5 seeds/60k) confirms adaptivity (`frac_std=0.192`) but not pure anticipation: reactive strength 0.130 > hazard strength 0.074. Claim boundary: dynamic resource-aware efficiency under realized risk/hazard, not clean pre-shock forecasting. This is now the lead paper lane. Artifacts: `outputs/experiments/preventive_pareto_hc0.0_2026-06-27`, `outputs/experiments/preventive_pareto_excel_delta_5seed_40k_2026-06-27`, `outputs/experiments/preventive_pareto_excel_delta_5seed_60k_2026-06-28`, `outputs/experiments/preventive_pareto_excel_delta_mixed10seed_40k_2026-06-28`, `outputs/experiments/preventive_pareto_excel_delta_mixed10seed_60k_2026-06-28`, `outputs/kaggle/scresia-preventive-pareto-final/scresia_preventive_pareto_final_outputs/decision.json`, `outputs/audits/prevention_anticipation_excel_delta_5seed_60k_2026-06-28/summary.json`, `outputs/experiments/preventive_pareto_results_summary_2026-06-28.json`. |
...
1. ⭐ Δmemory (`retained-reset`) on the lead continuous lane (`continuous_its + risk_obs/hazard + ReT_excel_delta + war`). This is the strongest remaining test for continuous learning.
2. Generalization of the lead Pareto lane across regimes/horizons after the h104 claim is frozen.
3. continuous_its + learned initial prepositioning (two-phase).
4. DMLPA + frame-stack + masked × continuous_its (Garrido learn-the-disruption).
5. Track B only after fixing/understanding compressed fill scale and 7D heuristic mismatch.
6. variance_log CD eval bar (port from postfix-reruns) to re-open the CD lane honestly.
7. n_envs 4–8 + norm-reward as a learning-stability upgrade on any lane.
