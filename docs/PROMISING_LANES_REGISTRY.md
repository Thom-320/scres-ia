...
1. ⭐ Re-check the v8 `ReT_excel_plus_cvar alpha=0.2` lane against the densest fair frontier; it remains the best surviving signal after per-op R2 campaign weakened.
...
3. per_op_buffer only if a new static structural gate shows true Op9-buffer movement; the R2-only campaign gate did not.
4. Generalization of the lead Pareto lane across regimes/horizons after the h104 claim is frozen.
5. continuous_its + learned initial prepositioning (two-phase).
6. DMLPA + frame-stack + masked × continuous_its (Garrido learn-the-disruption).
7. Track B only after fixing/understanding compressed fill scale and 7D heuristic mismatch.
8. variance_log CD eval bar (port from postfix-reruns) to re-open the CD lane honestly.
9. n_envs 4–8 + norm-reward as a learning-stability upgrade on any lane.
