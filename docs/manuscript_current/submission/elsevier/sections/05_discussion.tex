...
but it is not a strict cost win on the shift dimension: its
shift-utilization cost index is marginally
...
Pareto non-dominated on the ReT--cost--tail--flow frontier. The
...
the best static once dispatch expediting carries a moderate positive
charge --- the policy buys recovery with \emph{targeted}, not free,
...
provides mechanism evidence that downstream dispatch access is sufficient
to recover the largest observed advantage and that the number
of controllable dimensions is not what drives the Track B
