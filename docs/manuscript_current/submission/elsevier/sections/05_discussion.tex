...
\hypertarget{50-terminal-interpretation}{%
\subsection{Terminal interpretation: comparator scope, not adaptive superiority}\label{50-terminal-interpretation}}

The same-contract challenge in Section~\ref{44b-same-contract-challenge}
governs interpretation of all earlier positive Track B comparisons. The
147-cell frontier and local 3$\times$3 bound are valid restricted-family
benchmarks, but they do not span the learner's full decision contract. Once a
constant policy is calibrated over that full contract, its held-out Excel ReT
exceeds the ten-checkpoint PPO mean with a two-way confidence interval wholly
below zero for PPO minus static. We therefore make no claim that PPO is
adaptively superior in Track B. The scientific result is an audit result:
apparently robust learning gains can survive seed expansion, crossed tapes,
operational metrics, and privileged-information controls yet still disappear
when comparator decision rights are aligned with learner decision rights.
...
recovery-tail metrics than the best of a restricted dense downstream-dispatch grid of static comparators,
...
The identified factorial (Section~4.4) refines this restricted-family
interpretation, but the terminal same-contract challenge supersedes it as a
superiority gate. At matched 5-seed, 60{,}000-timestep scale on fresh
...
frontier that already optimizes exactly that subspace. This decomposition
shows why the original comparison was underpowered: most apparent value lay
in dimensions that the static family was not allowed to move. The anchored-
arm rerun retains a small within-learner dispatch increment, but the full-
contract static reversal prevents interpreting that increment as bottleneck
value over strong static control.
...
Our benchmark demonstrates a cautionary instance of this connection: when the
...
dispatch authority is exposed (Track B), a standard PPO+MLP agent produces a
decisive advantage only against a comparator family with narrower decision
rights; the advantage disappears under the same-contract challenge. The reward
...
succeeds. This suggests a practical design principle: \emph{before attributing
a gain to learning, grant the non-learning benchmark the same feasible
decision rights and separate the value of the contract from the value of
adaptation.}
