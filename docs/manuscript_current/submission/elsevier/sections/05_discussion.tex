...
recovery-tail metrics than the best of a dense grid of static comparators,
but it is not a strict cost win: its assembly cost index is marginally
higher than the best comparator selected by ReT, and this difference is
not statistically distinguishable from zero. We report the result as
Pareto non-dominated on the ReT--cost--tail--flow frontier rather than as
resource-efficient.
...
provides causal evidence that downstream dispatch access, not the number
of controllable dimensions, is the mechanism responsible for the Track B
advantage.
...
PPO+MLP agent can produce a decisive advantage in the designed sustained-disruption
cell. The reward sweep (Section~4.6) reinforces this: all 18 screened
reward/observation cells show positive Excel ReT deltas, whereas in Track A
no reward formulation succeeds. This suggests a
...
investigation. Second, the observation ablation removes the seven explicitly
privileged regime/forecast fields, but the remaining state still contains
rich operational feedback; the experiment rules out the strongest privileged
observation confound but does not prove full field deployability of every
state variable.
...
and emerging KAN-based designs as proposed by Garrido et al.~\cite{garrido2024iccl}---remain relevant for supply chains with variable topology or
...
identifies severe disruption at short horizons as a genuine boundary case;
...
Retained cross-campaign learning, off-policy algorithm comparisons
