...
Local DQN sensitivity:

- script: `/tmp/run_dqn_ladder_2026_06_26.sh`
- first active run:
  - label: `nightly_envb_aggr_g24_raw_ladder_10seed_24block_2026-06-26`
  - reward: `ReT_garrido2024_raw`
  - outcome: `excel_ret`
  - `phi=2.0`, `psi=1.5`
- this is a sensitivity path, not the primary frontier after the latest static
  and reward gates.
...
frontier cell, so the paper should not lead with a memory-retention win unless
another held-out cell overturns that result.
