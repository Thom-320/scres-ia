#!/usr/bin/env python3
"""Detached watcher for the frozen Paper-2 <=4-switch calibration gate."""
...
def snapshot(
    ...
    *,
    ...
    tree_rss = sum(int(row["rss_bytes"]) for row in tree)
    peak_rss = max(int(peak_rss_bytes), tree_rss)
    memory_available = memory_available_bytes()
    if memory_available is None:
        minimum_memory = min_memory_available_bytes
    elif min_memory_available_bytes is None:
        minimum_memory = memory_available
    else:
        minimum_memory = min(int(min_memory_available_bytes), memory_available)
    progress_age = (
    ...
    payload: dict[str, Any] = {
        ...
        "scientific_process_tree_rss_bytes": tree_rss,
    ...
    if pid_payload is None:
        ...
        payload["state"] = (
            "completed_unverified" if complete else "failed_or_incomplete"
        )
    return payload
...
def main() -> int:
    ...
    while True:
        payload = snapshot(
            run_dir,
            watcher_started=started_utc,
            peak_rss_bytes=peak_rss,
            min_memory_available_bytes=minimum_memory,
        )
        peak_rss = int(payload["peak_scientific_process_tree_rss_bytes"])
