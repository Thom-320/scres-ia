#!/usr/bin/env python3
"""Generate the shared Kaggle review/experiment notebook: TAIL-resilience PPO-vs-static.

Two audiences:
  1. Thom + Claude/Codex: the rigorous tail-resilience test (does PPO separate from the
     best static on the TAIL of the resilience distribution, even though it ties on the
     saturated mean?).
  2. David: a reviewable, knob-driven notebook to (a) see the faithful-env changes we
     made, (b) read the CORRECT resilience metrics (not the saturated mean), and (c)
     run his own experiments — including recurrent_ppo / dmlpa_ppo (his hypothesis that
     an MLP is too weak to anticipate disruptions) and panel training — to propose ways
     to make the model converge.

Metric guidance (Codex clarification, commit d2c5703 `Clarify ReT metric surfaces`):
  - PRIMARY all-order ReT  = `ret_mean_all_orders_zero_unfulfilled_mean`
    (legacy `order_level_ret_mean` only averages COMPLETED orders -> optimistic; do NOT
     judge victory by it, nor by the per-step training reward).
  - TAIL / disruption       = `ret_p10_all_mean`, `flow_fill_rate_mean`,
                              `stockout_week_pct_mean`, `pct_case_fill_rate_mean`.
  - WHERE resilience comes from = `re_fr/ap/rp/dp_rp_contribution_all_mean`
    (fill / autotomy / recovery / non-recovery decomposition).

The smoke (`run_thesis_decision_ppo_smoke.py`) already emits all of these per policy for
PPO and the 18-cell static I x S grid, evaluated on the SAME stress env with EQUAL eval
reps (avoids the asymmetric-reps artifact that inflated 3 prior "PPO wins").

This kernel is CPU-only (enable_gpu=false, device=cpu): the DES sim is the CPU-bound
bottleneck, so GPU does not help, and it is immune to the Kaggle P100 (sm_60) /
PyTorch sm_70+ incompatibility that broke the GPU history-screen kernel.
"""
...
CELL_KNOBS = code(
    ...
    "REWARD_MODE = 'ReT_tail_v1'     # tail/recovery-aligned (PASSES the reward_surface_audit:\n"
    "                                #   its best static is also good on ret_p10). 'ReT_ladder_v1'\n"
    "                                #   FAILS (tracks the mean -> argmax I504_S3 has floored tail).\n"
    "LADDER_W_SC, LADDER_W_RC, LADDER_W_EF = 0.65, 0.30, 0.05   # only used if REWARD_MODE=ReT_ladder_v1\n"
    "# ReT_tail_v1 knobs (recovery-dominant + UN-gated cost; cost kappas make I504_S3 lose):\n"
    "TAIL_W_SC, TAIL_W_RC, TAIL_W_CE = 0.30, 0.60, 0.10\n"
    "TAIL_CAP_KAPPA, TAIL_INV_KAPPA, TAIL_BOOST = 0.40, 0.25, 0.0\n"
    "TAIL_TRANSFORM = 'identity'  # 'identity' | 'power' | 'exp_norm'\n"
    "TAIL_GAMMA = 1.0             # used when TAIL_TRANSFORM='power' (try 1.25, 1.5, 2.0)\n"
    "TAIL_BETA = 2.0              # used when TAIL_TRANSFORM='exp_norm' (try 2, 4)\n"
    ...
    "ORDER_UP_TO_MULTIPLIER = 2.0   # thesis-faithful backbone.\n"
    "                               #   m2.0 reproduces Table 6.10 best (~738k/year, -3.8% vs ECS)\n"
    "                               #   and is the Paper-1 setting. m1.0 is a declared\n"
    ...
    "GIT_BRANCH = " + repr(GIT_BRANCH) + "   # tail wiring lives ONLY on this branch\n"
    "REPO_NAME = 'scresia'\n"
...
CELL_HELPERS = code(
    ...
    "        '--ret-tail-w-sc', str(TAIL_W_SC), '--ret-tail-w-rc', str(TAIL_W_RC),\n"
    "        '--ret-tail-w-ce', str(TAIL_W_CE), '--ret-tail-cap-kappa', str(TAIL_CAP_KAPPA),\n"
    "        '--ret-tail-inv-kappa', str(TAIL_INV_KAPPA), '--ret-tail-boost', str(TAIL_BOOST),\n"
    "        '--ret-tail-transform', str(TAIL_TRANSFORM), '--ret-tail-gamma', str(TAIL_GAMMA),\n"
    "        '--ret-tail-beta', str(TAIL_BETA),\n"
...
def main() -> None:
    ...
    nb_path = OUT_DIR / "scresia_tail_resilience.ipynb"
    nb_path.write_text(json.dumps(nb, indent=1), encoding="utf-8")
    meta = {
        "id": KERNEL_ID,
        "title": KERNEL_TITLE,
        "code_file": "scresia_tail_resilience.ipynb",
        "language": "python",
