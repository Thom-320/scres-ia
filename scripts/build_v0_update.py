...
# Equation
add_equation('r\u209C = \u2212( w_bo \u00d7 B\u209C/D\u209C + w_cost \u00d7 (S\u209C \u2212 1) )')
...
for num, item in enumerate([
    ('Regime-dependent value.', ' The adaptive controller is competitive under moderate stress and stronger under severe stress. This is consistent with the expectation that fixed policies become insufficient when disruption intensity exceeds their design point.'),
    ('Cost-aware adaptation.', ' The PPO agent does not simply default to the most expensive shift configuration. Under both scenarios, it uses a heterogeneous shift allocation, indicating that the reward function successfully incentivizes cost-aware behavior.'),
    ('Service preservation.', ' In neither scenario does the adaptive policy degrade service relative to the best static baseline. This confirms that the control_v1 reward function avoids the misalignment observed with ReT_thesis (Section 3.3.4).'),
], 1):
    p = doc.add_paragraph()
