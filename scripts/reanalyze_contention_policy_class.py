...
def episode_adaptive(seed, risks, freq, impact, *, rule,
    ...
    while not done:
        ...
        _, _, done, _ = sim.step(action={"cssu_allocation_a": float(target)},
...
def main() -> int:
    ...
    for key, arms in cells.items():
        ...
        results[key] = {
            ...
            "label_swap_max_abs_diff": float(np.max(np.abs(cl - sw))),
    ...
    placebo_beaten = all(r["clairvoyant_minus_constant"]["mean"]
                         > r["placebo_minus_constant"]["mean"] for r in results.values())
    equivariant = all(r["label_swap_max_abs_diff"] < 1e-6 for r in results.values())
        ...
        "f2_placebo_does_not_reproduce": {
            ...
            "evidence": {"why_it_can_fail": "op12 already showed an uninformed placebo beating a "
                         ...
                         "placebo_minus_constant": {k: r["placebo_minus_constant"]
                                                    for k, r in results.items()}}},
        "f3_label_swap_equivariance": {
            "passed": bool(equivariant),
            "evidence": {"why_it_can_fail": "a tie-break, queue order or hash bias favouring A "
                                            "would masquerade as a mechanism",
                         "max_abs_diff": {k: r["label_swap_max_abs_diff"]
                                          for k, r in results.items()}}},
    ...
    verdict = ("POLICY_CLASS_WAS_THE_BINDING_CONSTRAINT" if clair_wins and placebo_beaten
    ...
    for key, r in results.items():
        ...
        print(f"  equivarianza A<->B (max|d|)  {r['label_swap_max_abs_diff']:.2e}")
    print(f"\n  veredicto: {verdict}\n")
