#!/usr/bin/env python3
"""Timing value with budget AND intensity held identical, over an EXACT schedule class.

Contract: docs/ENMIENDA_BLOQUEO_V1_Y_PREREGISTRO_V2_TIMING_2026-08-08.md, part B, committed before
this file existed. Custody: declared replay. Adjudicates nothing, authorises no learner.
...
MAX_STEPS = 26
STEP_HOURS = 168.0
K_SURGE = 13                       # exactly half the horizon, at S2 -- never S3
SHIFT_SIGNAL = {1: -1.0, 2: 0.0}   # S3 is excluded so intensity cannot vary
BUFFER_FRAC = 0.0

CELLS = {"R24_current": {}, "R24_increased": {"R24": "increased"}}
...
def main() -> int:
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("results/exact_timing_headroom_v2/result.json"))
    args = ap.parse_args()
    ...
    falsifiers = {
        "f1_exactly_K_surge_weeks": {
            "passed": all(v["surge_weeks_by_class"][c] == [K_SURGE]
                          for v in results.values()
                          for c in ("schedules", "rule", "placebo", "ranked")),
            "evidence": {"why_it_can_fail": "if any policy plays a different number of surge weeks, "
    ...
    payload = {
        "schema_version": "exact_timing_headroom_v2",
        ...
        "supersedes": {"path": "results/budgeted_timing_headroom/result.json",
                       "relabelled": "BLOCKED_BY_DESIGN_AUDIT_DEVELOPMENT_ONLY",
                       "retained": True},
        "decision_rule": {
        ...
        "design": {"K_surge_weeks": K_SURGE, "max_steps": MAX_STEPS,
                   "shift_levels": [1, 2], "S3_excluded_to_isolate_timing": True,
                   "n_exact": len(exact), "n_enriched": len(enriched) + 2,
                   "seeds": seeds, "cells": list(CELLS), "k_holm": k_total},
        "risk_estimand": {"formula": "Delta(R24 increased) - Delta(R24 current)",
                          "value": delta_r24,
                          "note": ("R22/R23 are no longer decisive negative controls: the source "
                                   "does not establish they are causally immune to an upstream "
                                   "shift change")},
    ...
    digest = seal_and_write(payload, args.output, contract=args.contract,
                            reference=Path("results/budgeted_timing_headroom/result.json"))
