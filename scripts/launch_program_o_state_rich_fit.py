#!/usr/bin/env python3
"""Launch the frozen state-rich fit gate with watcher-first session custody."""
...
def main() -> int:
    ...
    if not parent_result.is_file() or sha256(parent_result) != str(
        ...
        failures.append("parent result missing or hash mismatch")
    else:
        seeds = json.loads(parent_result.read_text()).get("seeds")
        if seeds != list(range(7420001, 7420049)):
            failures.append("parent result is not the frozen burned-fit seed block")

    manifest = {
        "schema_version": "program_o_state_rich_fit_launch_manifest_v1",
        ...
        "parent_result_sha256": sha256(parent_result) if parent_result.is_file() else None,
    ...
    runner_command = [
        sys.executable,
        str(ROOT / "scripts/run_program_o_state_rich_fit_custodied.py"),
        "--run-dir",
        str(run_dir),
        "--run-id",
        str(args.run_id),
    ...
    control = {
        "schema_version": "program_o_state_rich_fit_producer_control_v1",
