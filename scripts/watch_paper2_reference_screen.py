#!/usr/bin/env python3
"""Independent detached watcher for the Paper-2 reference-calendar screen."""
...
def snapshot(run_dir: Path, *, watcher_started: str) -> dict[str, Any]:
    ...
    now = time.time()
    progress_age = (
        max(0.0, now - progress_path.stat().st_mtime)
        if progress_path.exists()
        else None
    )
    usage = shutil.disk_usage(run_dir)
    payload: dict[str, Any] = {
        "schema_version": "paper2_reference_screen_watcher_v1",
    ...
    if pid_payload is None:
        ...
        payload["state"] = (
            "running_fresh"
            if progress_age is not None and progress_age <= 45.0
        ...
        payload["state"] = (
            ...
            if progress_complete and payload["result_exists"]
...
def main() -> int:
    ...
    latest = run_dir / "watcher_latest.json"
    log = run_dir / "watcher.jsonl"
