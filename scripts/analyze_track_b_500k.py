...
def analyze(run_dir: Path) -> dict:
    ...
    def metrics(row: dict | None) -> dict:
        ...
        return {
            ...
            "ret": safe_float(row.get("order_level_ret_mean_mean", row.get("order_level_ret_mean"))),
    ...
    result = {
        ...
        "delta_ppo_vs_best_static": {
            "fill_pp": (ppo_m["fill"] - best_static["fill"]) * 100 if best_static["fill"] else None,
            "reward": ppo_m["reward"] - best_static["reward"] if best_static["reward"] else None,
        },
        "ppo_beats_s2": ppo_m["fill"] > s2_m["fill"] if s2_m["fill"] else None,
        "ppo_beats_best_static": ppo_m["fill"] > best_static["fill"],
        "shift_collapse": ppo_m["s1_pct"] > 80 or ppo_m["s2_pct"] > 80 or ppo_m["s3_pct"] > 80,
    }
    ...
    if SMOKE_DIR.exists() and (SMOKE_DIR / "policy_summary.csv").exists():
        ...
        if smoke_ppo:
            ...
            result["delta_500k_vs_smoke"] = {
                "fill_pp": (ppo_m["fill"] - smoke_m["fill"]) * 100 if smoke_m["fill"] else None,
                "reward": ppo_m["reward"] - smoke_m["reward"] if smoke_m["reward"] else None,
            }
...
def write_report(result: dict, output_dir: Path):
    ...
    if "error" in result:
        ...
        md_lines.append("Run `python scripts/analyze_track_b_500k.py` after the benchmark completes.\n")
    else:
        ppo = result["ppo"]
        s2 = result["s2_d1.00"]
        ...
        md_lines.append("| Policy | Reward | Fill Rate | Backorder | Order ReT | Shifts S1/S2/S3 |")
        md_lines.append("| --- | ---: | ---: | ---: | ---: | --- |")
        for name, m in [("**PPO**", ppo), ("s2_d1.00", s2), (f"best_static ({best['name']})", best)]:
            md_lines.append(
        ...
        md_lines.append(f"- PPO beats best static: **{result['ppo_beats_best_static']}**")
        ...
        if result["ppo_beats_best_static"] and not result["shift_collapse"]:
            ...
            md_lines.append("**PARTIAL.** PPO beats S2 but not best static. Modest improvement.")
        else:
            md_lines.append("**FAIL.** PPO does not beat baselines or shows shift collapse.")
