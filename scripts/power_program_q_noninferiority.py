#!/usr/bin/env python3
"""Program Q power analysis (frozen BEFORE opening any 749 seed).
...
c=2.24 (approximates the hierarchical simultaneous level; disclosed approximation). Selects the
smallest N in {128,160,192,256} with power >= 0.80 for E1 AND E2, else STOP_UNDERPOWERED.
"""
import json, sys
...
for cell in CELLS:
    ...
    lrn = np.stack([[fr[i, enc(c)] for i, c in enumerate(aud[s]["calendars"])]
                    for s in sorted(aud)])                              # (10, 48)
    panels[cell] = {"H_OL": lrn - ol[None, :], "D_N": lrn - cls[None, :]}
    print(cell, "obs H_OL", round(float((lrn - ol).mean()), 5),
          "obs D_N", round(float((lrn - cls).mean()), 5))

rng = np.random.default_rng(20260718)
out = {"schema_version": "program_q_power_v1", "c_critical": C_CRIT, "replications": M,
       "grid": GRID, "endpoints": {"E1": "LCB(H_OL)>=0.01 all cells",
       "E2": "CI(D_N) in [-0.01,+0.01] all cells"}, "power": {}}
selected = None
for N in GRID:
    e1 = e2 = 0
    for _ in range(M):
    ...
    if selected is None and e1 / M >= 0.80 and e2 / M >= 0.80:
        selected = N
out["selected_N"] = selected
out["verdict"] = f"N={selected}" if selected else "STOP_PROGRAM_Q_UNDERPOWERED_WITHIN_CAP"
Path(ROOT / "research/paper2_exhaustive_search/program_q_power_20260718.json").write_text(
    json.dumps(out, indent=1))
print("SELECTED:", out["verdict"])
