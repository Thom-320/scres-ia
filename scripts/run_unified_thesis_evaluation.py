...
def per_node_action(op3: int | None, op5: int | None, op9: int | None, shifts: int) -> np.ndarray:
    levels = [0 if p is None else THESIS_INVENTORY_PERIODS.index(int(p)) + 1 for p in (op3, op5, op9)]
    ...
    for period in [None, *THESIS_INVENTORY_PERIODS]:
        ...
        for shifts in (1, 2, 3):
            out.append((
                ...
                "thesis_factorized", "thesis_strict",
...
def l1b_candidates_from_screening(
    ...
    if "stage" not in df.columns:
        frames.append(df.sort_values("fill_rate_order_level_mean", ascending=False).head(top_k))
    else:
    ...
    for raw_name in df["policy"]:
        ...
        out.append((name, "factorized", "per_node", per_node_action(op3, op5, op9, shifts)))
    return out
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--panel-cfis", default="31-90", help="Scenario panel (Garrido Cf range).")
    p.add_argument("--replications", type=int, default=3, help="Reps per (policy, scenario).")
    ...
    p.add_argument("--l1b-screening-csv", default=None, help="Prior L1b policy_summary.csv for candidate actions.")
    ...
    p.add_argument("--ppo-run-root", default=None, help="Dir holding ppo_adaptive/<run>/ppo_mlp_thesis_decision.zip")
    p.add_argument("--include-garrido-oracle", action="store_true", default=True)
    p.add_argument("--no-garrido-oracle", dest="include_garrido_oracle", action="store_false")
...
def main() -> int:
    ...
    print(f"panel: {len(panel)} scenarios (Cf{panel[0]}..Cf{panel[-1]}), reps={args.replications}, max_steps={args.max_steps}")
    ...
    if args.include_l1a:
        for name, asm, ipm, act in l1a_candidates():
            candidates.append({"name": name, "kind": "static", "asm": asm, "ipm": ipm, "action": act, "space": "uniform"})
    if args.l1b_screening_csv:
        for name, asm, ipm, act in l1b_candidates_from_screening(
            ...
            candidates.append({"name": name, "kind": "static", "asm": asm, "ipm": ipm, "action": act, "space": "per_node"})
    ppo_models = {}
    if args.ppo_run_root:
        for name, asm, ipm, zp, vp, learn_initial_decision in discover_ppo_models(Path(args.ppo_run_root)):
            model, vec = load_ppo(zp, vp)
            ppo_models[name] = (model, vec)
            candidates.append({"name": name, "kind": "ppo", "asm": asm, "ipm": ipm, "model": model, "vec": vec,
                               "space": "uniform" if "E2a" in name else "per_node",
                               "learn_initial_decision": learn_initial_decision})
    print(f"candidates: {len(candidates)}  ({sum(c['kind']=='static' for c in candidates)} static, "
    ...
    for cand in candidates:
        for cfi in panel:
            spec = specs[cfi]
            override = {"enabled_risks": set(spec.enabled_risks), "risk_overrides": dict(spec.risk_overrides)}
            if cand["kind"] == "oracle":
                asm, ipm = "thesis_factorized", "thesis_strict"
            ...
            for rep in range(args.replications):
                eval_seed = args.base_seed + cfi * 1000 + rep  # depends ONLY on (cfi, rep)
                m = rollout(args=args, env_kwargs=ek, action_fn=action_fn, eval_seed=eval_seed)
                per_scenario_rows.append({
                    "policy": cand["name"], "kind": cand["kind"], "space": space,
                    "cfi": cfi, "source_cfi": spec.source_cfi, "family": spec.family,
                    "replication": rep, "eval_seed": eval_seed, **m,
                })
                done += 1
    ...
    agg = df.groupby(["policy", "kind", "space"]).agg(
    ...
    lines = ["# Unified thesis-decision evaluation", "",
    ...
    for _, r in agg.iterrows():
        lines.append(f"| {r.policy} | {r.space} | {r.fill_mean:.4f} | {r.ret_mean:.4f} | {r.reward_mean:.2f} | {int(r.n)} |")
    lines += ["", "## Headline comparison (best-in-space, same panel)", ""]
    if oracle_row:
        lines.append(f"- **Garrido oracle** (per-scenario matched): fill={oracle_row['fill_mean']:.4f}, ReT={oracle_row['ret_mean']:.4f}")
    if best_uniform:
        lines.append(f"- **Best static uniform (L1a)**: {best_uniform['policy']} fill={best_uniform['fill_mean']:.4f}, ReT={best_uniform['ret_mean']:.4f}")
    if best_per_node:
        lines.append(f"- **Best static per-node (L1b)**: {best_per_node['policy']} fill={best_per_node['fill_mean']:.4f}, ReT={best_per_node['ret_mean']:.4f}")
        if best_uniform:
            d = best_per_node["fill_mean"] - best_uniform["fill_mean"]
            lines.append(f"  - per-node minus uniform (fill): {d:+.4f}  -> per-node {'ADDS value' if d > 0 else 'does NOT add value'} on this panel")
