...
def format_cell_text(
    cell, bold=False, align=WD_ALIGN_PARAGRAPH.CENTER, font_size=FONT_SIZE_TABLE
):
...
def polish_table(table, col_widths_pct, header_color="D9E2F3", text_aligns=None):
    ...
    for row_idx, row in enumerate(table.rows):
        ...
        row.height_rule = (
            WD_ROW_HEIGHT_RULE.AT_LEAST if row_idx == 0 else WD_ROW_HEIGHT_RULE.AUTO
        )
        ...
        for col_idx, cell in enumerate(row.cells):
            ...
            if row_idx == 0:
                ...
                format_cell_text(
                    cell, bold=False, align=align, font_size=FONT_SIZE_TABLE
                )
