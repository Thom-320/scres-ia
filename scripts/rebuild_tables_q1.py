...
def set_cell_borders(cell, top=None, bottom=None):
    ...
    if top:
        parts.append(
            f'<w:top w:val="{top[0]}" w:sz="{top[1]}" w:space="0" w:color="{top[2]}"/>'
        )
    if bottom:
        parts.append(
            f'<w:bottom w:val="{bottom[0]}" w:sz="{bottom[1]}" w:space="0" w:color="{bottom[2]}"/>'
        )
...
booktabs(
    ...
    group_after_rows=[
        6,
        12,
    ],  # After inventory block (0-6), after binary indicators (8-12)
)
...
booktabs(
    ...
    bold_cells={
        (2, 2),
        (2, 3),
        (2, 4),  # S2 increased = best reward, fill, backorder
        (5, 2),  # S2 severe = best reward
        (6, 3),
        (6, 4),  # S3 severe = best fill, backorder
    },
