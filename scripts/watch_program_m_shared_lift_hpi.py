#!/usr/bin/env python3
"""Prestart and whole-session watcher for the Program M H_PI screen."""
...
def session_processes(session_id: int) -> list[dict[str, Any]]:
    """Return every member of a POSIX session, including reparented workers."""
...
def snapshot(
    ...
    *,
    ...
    progress_stale_seconds: float = 600.0,
) -> dict[str, Any]:
    pid_record = load_json(run_dir / "pid.json")
    scientific_dir = run_dir / "scientific_output"
    progress_path = scientific_dir / "progress.json"
    progress = load_json(progress_path)
    result_path = (
        Path(str(pid_record["result"]))
        if pid_record and isinstance(pid_record.get("result"), str)
        else scientific_dir / "result.json"
    )
    ...
    progress_result_hash = progress.get("result_sha256") if progress else None
    terminal_hash_matches = bool(
        result_hash is not None and progress_result_hash == result_hash
    )
    payload: dict[str, Any] = {
        "schema_version": "program_m_shared_lift_hpi_watcher_v1",
        ...
        "progress_result_sha256": progress_result_hash,
        "terminal_result_hash_matches": terminal_hash_matches,
    ...
    if pid_record is None:
        ...
        payload["state"] = (
            "running_fresh"
            if progress_age is not None and progress_age <= progress_stale_seconds
...
def main() -> int:
    ...
    if min(
        ...
        args.prestart_timeout_seconds,
        args.progress_stale_seconds,
    ) <= 0:
        parser.error("watcher intervals and stale threshold must be positive")
