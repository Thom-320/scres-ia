...
OUTPUT_ROOT = Path("/kaggle/working/scresia_track_a_continuous_costaware_outputs")
...
def main() -> None:
    ...
    label = f"kaggle_track_a_continuous_costaware_{PROFILE}_{started_at.strftime('%Y%m%dT%H%M%SZ')}"
    cfg = profile_args()
    ...
    sweep_cmd = [
        ...
        *risk_levels,
        "--pt-profiles",
        *env_list("SCRESIA_PT_PROFILES", "stoch_pt_hist"),
        "--use-cf-risk-profile",
        "--panel-cfis",
        cfg["panel_cfis"],
        "--train-timesteps",
    ...
    manifest = {
        ...
        "matrix": {
            ...
            "pt_profiles": env_list("SCRESIA_PT_PROFILES", "stoch_pt_hist"),
        },
        "stop_rule": {
            "promote_if": (
