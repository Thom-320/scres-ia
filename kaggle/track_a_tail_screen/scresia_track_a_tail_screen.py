...
OUTPUT_ROOT = Path("/kaggle/working/scresia_track_a_tail_outputs")
...
def main() -> None:
    ...
    label = f"kaggle_track_a_tail_{PROFILE}_{started_at.strftime('%Y%m%dT%H%M%SZ')}"
    cfg = profile_args()
    ...
    print("SCRESIA Track A tail/recovery screen", flush=True)
    ...
    manifest = {
        ...
        "purpose": (
            "Strict Track A tail/recovery screen after ReT_tail_v1 surface audit. "
            "Uses Garrido decision variables only."
        ),
        ...
        "matrix": {
            ...
            "action_space_modes": ["thesis_factorized"],
            "pt_profiles": ["stoch_pt_hist"],
        },
        "stop_rule": {
            "promote_if": (
                "any config improves best static by >=0.02 in all-order ReT or "
                "flow_fill, or improves ret_p10_all with lower stockout and no "
                "material fill loss"
            ),
            "otherwise": "close Track A before continuous_it_s / Track B",
        },
    }
