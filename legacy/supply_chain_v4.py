...
class MFSCSimulation:
    ...
    def step(self, action=None, step_hours=None):
        """
        ...
        # Compute reward
        new_backorders = self.total_backorders - prev_backorders
        new_delivered = self.total_delivered - prev_delivered
        reward = new_delivered - 10 * new_backorders  # Configurable
    ...
    def _op1_contracting(self):
        while True:
            ...
            yield self.env.timeout(self.params["op1_pt"])
    ...
    def _op2_supplier_delivery(self):
        while True:
            ...
            while self._is_down(2):
                yield self.env.timeout(1)
            yield self.env.timeout(self.params["op2_pt"])
    ...
    def _op3_wdc_dispatch(self):
        while True:
            ...
            if dispatch > 0:
                ...
                yield self.env.timeout(self.params["op3_pt"])
    ...
    def _op8_transport_to_sb(self):
        ...
        while True:
            ...
            while self._is_down(8):
                yield self.env.timeout(1)
            yield self.env.timeout(self.params["op8_pt"])
            yield self.rations_sb.put(batch_size)

    def _op9_sb_dispatch(self):
        """Op9: Supply Battalion — dispatch up to U(q_min, q_max) from available stock."""
        while True:
    ...
    def _op10_deliver(self, qty):
        """Async delivery for Op10 (PT=24h transit)."""
        yield self.env.timeout(self.params["op10_pt"])
        yield self.rations_cssu.put(qty)

    def _op12_transport_to_theatre(self):
        """Op12: LOC CSSUs→Theatre — transport up to U(q_min, q_max), pipelined PT."""
        while True:
    ...
    def _op12_deliver(self, qty):
        """Async delivery for Op12 (PT=24h transit)."""
        yield self.env.timeout(OPERATIONS[12]["pt"])
    ...
    def _op13_demand(self):
        ...
        while True:
            ...
            demand_qty = self.rng.integers(DEMAND["a"], DEMAND["b"] + 1)
    ...
    def _get_risk_b(self, risk_id):
        if self.risk_level == "increased" and risk_id in RISKS_INCREASED:
            return RISKS_INCREASED[risk_id].get(
                "b", RISKS_CURRENT[risk_id]["occurrence"]["b"]
            )
        return RISKS_CURRENT[risk_id]["occurrence"]["b"]

    def _get_risk_p(self, risk_id):
        if self.risk_level == "increased" and risk_id in RISKS_INCREASED:
            return RISKS_INCREASED[risk_id].get(
                "p", RISKS_CURRENT[risk_id]["occurrence"]["p"]
            )
    ...
    def _risk_R14(self):
        ...
        while True:
            ...
            if produced > 0:
                ...
                if defects > 0:
                    self._pending_batch = max(0, self._pending_batch - defects)
    ...
    def _daily_tracker(self):
        while True:
            ...
            self.daily_production.append((self.env.now, self._today_produced))

    def get_annual_throughput(self):
        hours_per_year = 8_760
        yearly_produced = {}
        for t, qty in self.daily_production:
            year = int(t // hours_per_year) + 1
            yearly_produced.setdefault(year, 0)
            yearly_produced[year] += qty
        years = self.horizon / hours_per_year
        return {
            "produced_by_year": yearly_produced,
            "avg_annual_delivery": self.total_delivered / years,
            "avg_annual_production": self.total_produced / years,
        }
    ...
    def summary(self):
        ...
        print(f"  Granularity: HOURLY (assembly)  |  Seed: {self.seed}")
        print(f"{'='*60}")
