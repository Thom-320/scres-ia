...
RES = OUT.parent / "data" / "oracle_capture_v1"
...
static_mean = metric["bars"]["deployable_static_out_of_sample"]["mean_label"]
hindsight_mean = metric["bars"]["hindsight_static_in_sample"]["mean_label"]
...
rows = [
    ...
    (
        "Retained belief-MPC\n(structured feedback)",
        label_of("frozen_c256_mpc_retained"),
        ORANGE,
        "policy",
    ),
    (
        "Belief-reset MPC\n(feedback, no retention)",
        label_of("frozen_c256_mpc_reset"),
        ORANGE,
        "policy",
    ),
    ("Hindsight static\n(adversarial, in-sample)", hindsight_mean, GREEN, "anchor"),
    ("Deployable static\n(selected out of sample)", static_mean, GREEN, "bar"),
    (
        "Constant allocation 1:2\n(discretionary anchor)",
        label_of("constant_action_1"),
        MUTED,
        "anchor",
    ),
    (
        "Constant allocation 2:1\n(discretionary anchor)",
        label_of("constant_action_2"),
        MUTED,
        "anchor",
    ),
]

fig, (axa, axb) = plt.subplots(1, 2, figsize=(DC, 3.05), gridspec_kw={"width_ratios": [1.06, 1.0]})
...
for (name, value, color, kind), yy in zip(rows, y):
    ...
    axa.barh(
        ...
        height=0.56,
        color=color,
        alpha=0.30 if kind == "oracle" else 0.92,
        edgecolor=color,
        linewidth=0.9,
        hatch=hatch,
        zorder=3,
    ...
    axa.text(
        value + 0.004, yy, f"{value:.3f}", va="center", ha="left", fontsize=6.8, color=INK, zorder=4
    )
...
axa.set_title(
    "(a) Distance to the exact clairvoyant ceiling",
    fontsize=7.8,
    weight="bold",
    color=INK,
    loc="left",
...
for arch, (nice, color) in curves.items():
    ...
    axb.fill_between(
        steps, mat.min(axis=0), mat.max(axis=0), color=color, alpha=0.16, lw=0, zorder=2
    )
    axb.plot(steps, mean, color=color, lw=1.6, zorder=3, label=f"{nice} ({mat.shape[0]} seeds)")
    plotted += 1

mpc = pol["frozen_c256_mpc_retained"]["hindsight_static_in_sample"]["pooled"]["pooled_ratio"]
...
axb.text(
    0.985,
    mpc,
    "retained belief-MPC (+0.74 on historical hindsight-bar scale)",
    transform=axb.get_yaxis_transform(),
    ha="right",
    va="bottom",
    fontsize=6.0,
...
axb.text(
    0.985,
    0.0,
    "historical in-sample static bar",
    transform=axb.get_yaxis_transform(),
    ha="right",
    va="bottom",
    fontsize=6.2,
...
lo = min(
    ...
    min(
        ...
        for mat in [
            np.array(
                [
                    [p["pooled_ratio"] for p in c["points"]]
                    for c in json.loads((RES / f"learning_curve_{a}.json").read_text())["curves"]
                ]
            )
            for a in curves
            if (RES / f"learning_curve_{a}.json").exists()
        ]
        or [np.array([0.0])]
    ),
)
...
axb.set_title(
    "(b) Historical pilot (unmatched retention; hindsight-bar scale)",
    fontsize=7.4,
    weight="bold",
    color=INK,
    loc="left",
...
for ext in ("pdf", "png"):
    fig.savefig(OUT / f"fig7_oracle_metric.{ext}", dpi=300, bbox_inches="tight", facecolor=SURF)
plt.close(fig)
