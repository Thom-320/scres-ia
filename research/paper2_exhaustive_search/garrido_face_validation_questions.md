...
**Licenses a domain-valid M/T/R extension if:** such a single mutually-exclusive resource exists with a real identity and causal effect on recovery time at each target. The family would still need an exact same-contract frontier or valid resource-restricted ceiling under the real resource semantics.
