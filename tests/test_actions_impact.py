...
if __name__ == "__main__":
    ...
    run_episode_with_action(
        np.array([-1.0, -1.0, -1.0, -1.0]), "EXTREME STARVATION (Half Inventory)"
    )
    run_episode_with_action(
        np.array([0.0, 0.0, 0.0, 0.0]), "BASELINE (1.25x Inventory)"
    )
    run_episode_with_action(
        np.array([1.0, 1.0, 1.0, 1.0]), "EXTREME HOARDING (Double Inventory)"
    )
